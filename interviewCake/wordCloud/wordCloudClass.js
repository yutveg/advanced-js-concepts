class WordCloudData {
  ...
  populateWordsToCounts() {
    ...
    for (let i = 0; i < this.inputString.length; i++) {
      const character = this.inputString.charAt(i);
      ...
      if (i === this.inputString.length - 1) {
        ...
        if (currentWordLength > 0) {
          const word = this.inputString.slice(
        ...
        if (
          i < this.inputString.length - 1 &&
          this.inputString.charAt(i + 1) === "."
        ) {
          if (currentWordLength > 0) {
            const word = this.inputString.slice(
              currentWordStartIndex,
        ...
        if (
          i > 0 &&
          this.isLetter(this.inputString.charAt(i - 1)) &&
          this.isLetter(this.inputString.charAt(i + 1))
        ) {
          if (currentWordLength === 0) {
            currentWordStartIndex = i;
